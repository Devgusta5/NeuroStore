  ...
  return (
    <section className="py-20 px-4 relative">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div
          className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-blue-500/5 rounded-full blur-3xl animate-float-slow"
      ...
      <div className="max-w-7xl mx-auto relative">
        <div className="text-center mb-16">
          ...
          <p className="text-violet-600 dark:text-slate-300 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
            Trechos de códigos prontos para uso para tarefas comuns de
            reconhecimento neural e por gestos.
          </p>
        </div>
          ...
          {codeExamples.map((example, index) => {
            ...
            return (
              ...
              >
                {/* Gradient Overlay */}
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${example.color} opacity-0 group-hover:opacity-10 transition-opacity duration-500`}
                ></div>

                {/* Animated Border */}
                <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  <div
                    className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${example.color} p-[1px]`}
                  >
                ...
                <div className="relative p-8">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-4">
                      <div
                        className={`p-3 rounded-xl bg-gradient-to-br ${example.color} group-hover:scale-110 transition-transform duration-300`}
                      >
                    ...
                    <span
                      className={`px-3 py-1 bg-gradient-to-r ${example.color} text-white text-xs rounded-full font-medium shadow-lg`}
                    >
                      {example.category}
                    </span>
                  </div>
        ...
        <div className="text-center mt-16">
          <button className="relative bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white px-8 py-4 rounded-xl font-semibold transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-purple-500/25 group overflow-hidden">
            <span className="relative z-10">Ver todos os exemplos</span>
